...
$(function(){
	...
	$("#join-submit").click(function(){
		...
		pwd_chk();
		email_chk();
		if($("#join-final-chk").is(":checked")){
			agree_checked = 1;
		}else{
			agree_checked = 0;
			$(".agree-valid").html("위 사항에 동의해야만 회원 가입이 가능합니다.");
		}
		if(cnt == 10 && id_checked == 1 && pwd_checked == 1 && email_checked == 1 && agree_checked == 1){
			insert_member();
		}
	});
...
// 		});
	...
	function join_form_chk(){
		$(".join-form").each(function(index,data){
			...
			if(id_checked == 0){
				$("#m_id").css("background","#faffbd");
				$(".id-valid").html("아이디 중복 확인 버튼을 먼저 누르세요.");
			}
			if(email_checked == 0){
	...
	function id_chk(m_id){
		if(m_id.length != 0 && (m_id.length >= 6 && m_id.length <= 10)){
			var id_chk_form = new FormData();
			id_chk_form.append("m_id",m_id);
			$.ajax({
				url:'memberIdChk.do',
				data:id_chk_form,
				method:"post",
				...
				success:function(data){
					var res = eval("("+data+")");
					if(res == "o"){
						$(".id-valid").html("사용 가능한 ID입니다.");
						id_checked = 1;
					}else{
						$(".id-valid").html("이미 존재하는 ID입니다.");
						id_checked = 0;
					}
			...
			$(".id-valid").html("아이디는 6자 이상 10자 이하이어야 합니다.");
			id_checked = 0;
			return false;
		}
...
<div class="join-body">
	<div class="join-title">
		...
		<div class="title">
		ID <span style="font-size:0.95em;">생성</span>
		</div>
	</div>
	<div class="join-body">
	<form action="join.do" method="post" id="join-form">
		<div class="join-body-top join-block">
			<div class="join-td" style="text-align:center;margin-bottom:2%;">
				...
				<span style="font-famliy:'Whitney';font-weight: bold;">Gottube ID</span><span style="font-family:'Noto Sans Kr';">를 가지고 계십니까?</span>
				<a href="#" class="join-link">찾아보기 <span style="font-family:'Oswald';">></span></a>
			</div>
			<div class="join-td" style="margin-top:2%;">
				<input id="m_id" type="text" style="width:86%;display:inline-block;margin-bottom:0;" name="m_id" placeholder="ID" class="form-control join-form">
				<span id="id-chk" class="id-chk">
				확인
				</span>
				<span class="valid-chk id-valid"></span>
			</div>
			<div class="join-td">
				<input id="pwd" type="password" name="pwd" placeholder="암호" class="form-control pwd-form join-form">
			</div>
		...
		<div class="join-body-middle join-block">
			<div class="join-td">
				<input type="text" name="last-name" style="width:48.5%;display:inline;" placeholder="성" class="form-control join-form">
				<input type="text" name="first-name" style="width:48.5%;float:right;display:inline;" placeholder="이름" class="form-control join-form">
			</div>
			<div class="join-td">
				<input type="text" name="phone" placeholder="전화번호" class="form-control join-form" style='ime-mode:disabled;' onkeydown='return onlyNumber(event)' onkeyup='removeChar(event)'/>
			</div>
			<script type="text/javascript">
			...
			<div class="join-td">
				<input id="email" type="text" name="email" placeholder="e-mail" style="margin-bottom:0;" class="form-control join-form">
		...
		<div class="join-body-middle2 join-block" align="center">
			<div class="join-td">
				...
				<div class="profile-preview">
					<img src="resources/upload/check1.jpg" width="100%" height="100%" id="profile-img" style="margin-top:30px;" chk="no"/>
					<input type="hidden" value="" name="profile" id="profile-val"/>
				</div>
		...
		<div class="join-body-bottom join-block">
			<div class="join-td">
				...
				<input type="checkbox" id="join-final-chk" style="top:2em;">
				<span style="font-family:'Noto Sans Kr';">
				<span style="font-famliy:'Whitney';font-weight: bold;">Gottube</span>
				의 개인정보 취급방침에 따라 개인 정보의 수집, 사용, 타사에 대한 제공 및 처리에 동의합니다.
				</span>
				<span class="valid-chk agree-valid"></span>
			</div>
