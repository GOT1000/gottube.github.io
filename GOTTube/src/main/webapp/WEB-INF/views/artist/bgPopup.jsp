...
$(function(){
	...
	function fileTempUpload(){
		...
		formData.append("source","artist_temp/bg");
	...
	function fileUpload(){
		var source = "artist/bg";
		var x = $("#x").val();
		var y = $("#y").val();
		var w = $("#w").val();
		var h = $("#h").val();
		var i = $("#i").val();
		var og = $(opener.document).find(".bg-og").val();
		...
		$.ajax({
			...
			success:function(data){
				...
				$(opener.document).find("#bg-img").attr("src",img);
				$(opener.document).find("#bg-img").attr("chk","yes");
				$(opener.document).find(".bg-val").val(img);				
				$(opener.document).find(".bg-og").val(img);
...
.display-none{ /*감추기*/
	...
	<div class="profile-popup">
		<div align="center" style=" padding:10px;">
			<label style="font-family:Arial Black;font-size:30px;">배경 업로드</label>
		</div>
